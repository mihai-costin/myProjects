var count = 0, win = 0; // count for mistakes, win for matching letters
var obj = [
    {
        guess: "red",
        hint: "color"},
...
var rdn, choice, play = false, size;
var verify = []; // binary array to verify if a letter from a string was already checked
...
function check(clicked){
    ...
    for(var i=0;i<choice.length;i++)
        {    if(clicked === choice[i] && verify[i] === 0)
                { // if it's matching and it was not pressed before
              ...
              else if(clicked === choice[i] && verify[i] === 1 && bool != 1){
                  // if the letter was pressed before
                  // bool != 1 in order for this not to trigger more then one time
...
function keyPressed(event){
    ...
    else alert("Press a key from a to z!");
